...
TEST(table_storage, multi_thread)
{
    ...
    for (size_t i = 0; i < query_results.size(); i++) {
        ...
        ASSERT_TRUE(query_results[i][g_query_attr_id] >= g_lower_bound && query_results[i][g_query_attr_id] <= g_upper_bound);
    }
...
TEST(table_storage, single_thread1)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (size_t i = 0; i < query_results.size(); i++) {
            ASSERT_EQ(query_results[i].size(), Table::kNumTableAttributes);
            ASSERT_TRUE(query_results[i][query_attr_id] >= lower_bound && query_results[i][query_attr_id] <= upper_bound);
        }
    }
    end = clock();

...
Table::Table(const std::string& table_file_name,
...
Table::~Table() {
  ...
  if (index_file_readonly_ != nullptr) {
    delete index_file_readonly_;
  }
  if (table_file_readonly_ != nullptr) {
    delete table_file_readonly_;
  }
  if (env_ != nullptr) {
    delete env_;
  }
...
Status Table::BuildIndexBlock(int attr_id) {
  ...
  for (int i = 0; i < nr_entries_; i++) {
    ...
    index_entries.push_back(std::make_pair(DecodeFixed64(result.data()), i));
  ...
  if (status.ok()) {
    ...
    if (status.ok()) {
      ...
      if (index_file_readonly_ == nullptr) {
        status =
            env_->NewRandomAccessFile(index_file_name_, &index_file_readonly_);
      }
    }
  }
  return status;
...
Status Table::Lookup(int attr_id, uint64_t lower_bound, uint64_t upper_bound,
  ...
  if (index_attr_id_ != attr_id ||
      index_file_readonly_ ==
          nullptr) { /* 索引不存在则读取table_file进行顺序查找 */
    if (table_file_readonly_ == nullptr) {
      status =
          env_->NewRandomAccessFile(table_file_name_, &table_file_readonly_);
      if (!status.ok()) {
        return status;
      }
    }
    ...
    for (int i = 0; i < nr_entries_; i++) {
      status =
          table_file_readonly_->Read(offset, sizeof(scratch), scratch, &slice);
      if (!status.ok()) {
        return status;
      }
  ...
  for (int i = 0; i < nr_entries_; i++) {
    ...
    status = index_file_readonly_->Read(offset + sizeof(uint64_t),
                                        sizeof(uint32_t), scratch, &slice);
    if (!status.ok()) {
      return status;
    }
  ...
  int lower_bound_idx =
      FindIndexEntryGreaterOrEqual(index_entries, lower_bound);
...
int Table::FindIndexEntryGreaterOrEqual(std::vector<IndexEntry>& index_entries,
                                        uint64_t x) const {
  ...
  while (left < right) {
    ...
    if (mid + 1 < index_entries.size() && index_entries[mid].first < x &&
        x <= index_entries[mid + 1].first) {
      return mid + 1;
...
int Table::FindIndexEntryLessOrEqual(std::vector<IndexEntry>& index_entries,
                                     uint64_t x) const {
  int left = 0, right = static_cast<int>(index_entries.size()) - 1;
  if (x < index_entries[0].first) {
    return -1;
...
Status Table::Recover() {
  ...
  if (status.ok()) {
    ...
    status = manifest_file->Read(sizeof(uint32_t), sizeof(uint32_t), scratch,
                                 &slice);
    if (!status.ok()) {
      return status;
    }

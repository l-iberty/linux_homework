...
class WindowsRandomAccessFile : public RandomAccessFile {
 public:
  WindowsRandomAccessFile(const std::string& filename, HANDLE file_handle)
      : filename_(std::move(filename)), file_handle_(file_handle) {
  ...
  Status Read(uint64_t offset, size_t nbytes, char* scratch,
...
class WindowsWritableFile : public WritableFile {
 public:
  ...
  ~WindowsWritableFile() override { Close(); }
  ...
  Status Flush() override {
    ...
    ::WriteFile(file_handle_, reinterpret_cast<LPCVOID>(buf_), pos_,
 ...
 public:
  ...
  Status GetFileSize(const std::string& filename, size_t* size) override {
    HANDLE file_handle = ::CreateFileA(
        filename.c_str(), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE,
  ...
  Status OpenWritableFile(const std::string& filename,
                          WritableFile** result) override {
    HANDLE file_handle =
        ::CreateFileA(filename.c_str(), GENERIC_READ | GENERIC_WRITE,
